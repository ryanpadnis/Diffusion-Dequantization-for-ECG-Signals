...
# ---------------------------------------------------------------------------
# Helpers for energy metrics serialization
# ---------------------------------------------------------------------------

def _serialize_energy_results(results: dict | None) -> dict | None:
    """Convert compute_energy_metrics output into JSON-serializable dict."""
    if not results:
        return None
    out: dict = {
        'threshold': results['threshold'],
        'gt_band_fracs': results['gt_band_fracs'],
        '4bit_vs_gt': results['4bit_vs_gt']._asdict(),
        'trajectories': [],
    }
    for entry in results.get('trajectories', []):
        m = entry.get('metrics')
        vs4 = entry.get('vs_4bit')
        out['trajectories'].append({
            ...
            'metrics': m._asdict() if hasattr(m, '_asdict') else {},
            'vs_4bit': vs4._asdict() if hasattr(vs4, '_asdict') else {},
        })
    return out
...
def main() -> None:
    ...
    if bool(args.all_samples):
        ...
        if bool(args.minimal) and metrics_all:
            ...
            # energy metric summary if available
            nefs: list[float] = []
            for m in metrics_all:
                em = m.get('energy_metrics')
                if em:
                    nef = em.get('4bit_vs_gt', {}).get('nef')
                    try:
                        nef = float(nef)
                    except Exception:
                        nef = float('nan')
                    if np.isfinite(nef):
                        nefs.append(nef)
            if nefs:
                print(f"minimal summary: mean NEF(4bit_vs_gt)={float(np.mean(nefs)):.4f} over {len(nefs)} samples")
        return
