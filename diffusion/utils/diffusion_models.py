...
class ConditionalDiffuser(nn.Module):
    def __init__(self, config: Dict[str, Any], unet_type: str = 'conditional', scheduler_type: str = 'ddpm'):
        super().__init__()
        self.config = config
    ...
    def _build_scheduler(self, scheduler_type: str, config: Dict[str, Any]):
        num_timesteps = int(config.get('num_noising_steps', 1000))
        beta_schedule = str(config.get('noise_schedule_type') or config.get('beta_type', 'linear')).strip().lower()
        ...
        if pred in {'x0', 'x_start', 'xstart', 'start', 'sample'}:
            ...
            raise ValueError(f"Unknown prediction_type: {pred!r}")

        trained_betas = None
        if beta_schedule in {'cosine', 'cos'}:
            diffusers_beta_schedule = 'squaredcos_cap_v2'
        elif beta_schedule == 'sigmoid':
            beta_min, beta_max = 1e-4, 0.02
            t = torch.linspace(0.0, 1.0, num_timesteps)
            sigmoid_t = 1.0 / (1.0 + torch.exp(-6.0 * (t - 0.5)))
            trained_betas = (sigmoid_t * (beta_max - beta_min) + beta_min).tolist()
            diffusers_beta_schedule = 'linear'
        else:
            diffusers_beta_schedule = beta_schedule

        kwargs = dict(num_train_timesteps=num_timesteps, prediction_type=pred)
        if trained_betas is not None:
            kwargs['trained_betas'] = trained_betas
        else:
            kwargs['beta_schedule'] = diffusers_beta_schedule
    ...
    def _build_condition_encoder(self, config: Dict[str, Any]) -> nn.Module:
        cond_in_channels = max(1, int(config.get('cond_in_channels', config.get('in_channels', 1)) or 1))
    ...
    def encode_condition(self, condition: torch.Tensor) -> torch.Tensor:
        ...
        return cond_features.view(B, C, H * W).permute(0, 2, 1)
    ...
    def forward(self, clean_images: torch.Tensor, condition: torch.Tensor) -> torch.Tensor:
        ...
        model_pred = self.unet(noisy_images, timesteps, encoder_hidden_states=cond_embed, return_dict=False)[0]

        pred = str(self.config.get('prediction_type', 'epsilon') or 'epsilon').strip().lower()
        target = noise if pred in {'eps', 'epsilon', 'noise'} else clean_images

        loss_type = str(self.config.get('loss_type', 'mse') or 'mse').strip().lower()
        if loss_type in {'mse', 'l2'}:
            return torch.mean((model_pred - target) ** 2)
        elif loss_type in {'l1', 'mae'}:
            return torch.mean((model_pred - target).abs())
        elif loss_type in {'huber', 'smooth_l1'}:
            beta = float(self.config.get('loss_huber_beta', 1.0) or 1.0)
            return F.smooth_l1_loss(model_pred, target, beta=beta)
